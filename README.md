...
- **高级功能** — 强劲(Turbo)/省电(Econo)/睡眠(Sleep)/灯光(Light)/自清洁(Clean)，格力 YBOFB 原生位编码 + 通用品牌 IRac 适配
...
| v3.1 | 完整 HVAC 控制：Turbo(强劲)/Econo(省电)/Sleep(睡眠)/Light(灯光)/Clean(自清洁)；格力 YBOFB 原生位编码（5 个功能位精确映射）；通用品牌 IRac 适配（30+ 品牌）；新增 HVAC2 UDP 协议（向后兼容老从机）；WebUI 5 个 iOS 风格 toggle 开关；状态持久化 |
